...
## `--prefix` Filter with String-Manipulation Operators
...
| Expression | Notes | No prefix (default) | --prefix APP_ |
|------------|-------|---------------------|---------------|
| **`${APP_WORD:5}`** | literal — always works | `world` | `world` |
| **`${APP_WORD:$APP_OFFSET}`** | index var matches prefix → evaluated | `world` | `world` |
| **`${APP_WORD:$OTHER}`** | index var not matching → verbatim | `helloworld` | `${APP_WORD:$OTHER}` |
| **`${APP_WORD#$APP_PAT}`** | pattern var matches prefix → evaluated | `world` | `world` |
| **`${APP_WORD#$OTHER}`** | pattern var not matching → verbatim | `helloworld` | `${APP_WORD#$OTHER}` |
| **`${APP_WORD%$OTHER}`** | suffix pattern not matching → verbatim | `helloworld` | `${APP_WORD%$OTHER}` |
